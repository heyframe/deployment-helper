...
use HeyFrame\Deployment\Tests\TestUtil\StaticSystemConfigHelper;
...
class AccountServiceTest extends TestCase
{
    ...
    {
        $output = $this->createMock(SymfonyStyle::class);
        $output->expects($this->once())->method('info')->with(static::stringContains('Refreshed global shop token to communicate to store.heyframe.com'));
    ...
    public function testLoginPayloadChanged(): void
    {
        ...
        $this->expectExceptionMessage('Got invalid response from HeyFrame API: {"message":"Invalid credentials"}');
        $service->refresh($output, '6.4.0-dev', 'old.example.com');
    }
