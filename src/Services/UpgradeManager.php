...
class UpgradeManager
{
    public function __construct(
        private readonly HeyFrameState $state,
    ...
    public function run(RunConfiguration $configuration, OutputInterface $output): void
    {
        ...
        $output->writeln('HeyFrame is installed, running update tools');

        $this->processHelper->console(['messenger:setup-transports']);

        if ($this->state->getPreviousVersion() !== $this->state->getCurrentVersion()) {
            $output->writeln(\sprintf('Updating HeyFrame from %s to %s', $this->state->getPreviousVersion(), $this->state->getCurrentVersion()));
